# Control-UCR'S - FULL FINAL - Render + Windows
import os, sqlite3
...
def init_db():
    ...
    if not fetchall("SELECT * FROM users"):
        ...
        execute("INSERT INTO users (username,password,role) VALUES (%s,%s,%s)",('John',generate_password_hash('1234'),'operador'))
...
def index():
    ...
    def card(m):
        ...
        return f"<div class='card-mov p-2 d-flex align-items-center'><div style='flex:1'><div><b>{safe(m,'nombre_tienda')}</b> <small class='text-muted'>({safe(m,'region')}) - {safe(m,'placa')}</small></div><small>{safe(m,'tipo_ucrs')} {safe(m,'tamano')} {safe(m,'color')} | Cant: <b>{orig}</b></small><br><small class='text-muted'>{safe(m,'fecha')} - {safe(m,'usuario')}</small></div><div style='width:170px' class='text-end'><label class='border rounded-pill px-2 py-1 bg-light' style='font-size:.75rem;cursor:pointer'><input type='checkbox' {chk} onchange=\"location.href='/verificar/{mid}/{t}'\"> check tienda</label><form method='post' action='/editar_cantidad'><input type='hidden' name='id' value='{mid}'><input type='number' name='cantidad_corregida' value='{mostrar}' class='form-control form-control-sm rounded-pill mt-1 text-center' title='Corregir cantidad' onchange='this.form.submit()'></form></div></div>"
    lista="".join([card(m) for m in eg]) or "<div class='p-4 text-center bg-white rounded'>Sin despachos aún. Dale CEDI → TIENDA</div>"
    role=session.get('role')
    botones="<div style='position:fixed;bottom:40px;left:0;right:0;background:#fff;padding:12px;display:flex;gap:10px;justify-content:center;z-index:1000;border-top:2px solid #eee'><a href='/nuevo?tipo=egreso' class='btn btn-dark px-4 fw-bold shadow'>CEDI → TIENDA</a><a href='/nuevo?tipo=ingreso' class='btn btn-outline-dark px-4 fw-bold shadow'>TIENDA → CEDI</a></div>" if role=='admin' else "<div style='position:fixed;bottom:40px;left:0;right:0;background:#E70A29;padding:12px;display:flex;justify-content:center;z-index:1000'><a href='/nuevo?tipo=ingreso' class='btn btn-light fw-bold px-5 shadow'>TIENDA-CEDI</a></div>"
...
def nuevo():
    ...
    if request.method=='POST':
        ...
        return redirect('/')
    import json; tiendas_json=[{"region":safe(t,'r_nombre'),"tienda":safe(t,'t_nombre')} for t in tiendas_all]; opciones=''.join([f"<option value='{safe(r,'nombre')}'>{safe(r,'nombre')}</option>" for r in regiones])
    html=f"<div class='card p-4 mx-auto shadow' style='max-width:540px;border-radius:14px'><h5 class='fw-bold' style='color:#E70A29'>{'CEDI → TIENDA' if tipo=='egreso' else 'TIENDA → CEDI'}</h5><form method='post'><input type='hidden' name='tipo' value='{tipo}'><select id='regionSelect' name='region' class='form-select mb-2' required><option value=''>Seleccione Región</option>{opciones}</select><select id='tiendaSelect' name='nombre_tienda' class='form-select mb-3' required><option value=''>Seleccione Tienda</option></select><div class='row g-2 mb-2'><div class='col-4'><input name='cantidad' type='number' class='form-control' placeholder='Cantidad' required></div><div class='col-8'><input name='placa' class='form-control' placeholder='Placa vehículo' required></div></div><div class='row g-2 mb-2'><div class='col-4'><input name='tipo_ucrs' class='form-control' placeholder='Tipo UCR'></div><div class='col-4'><input name='tamano' class='form-control' placeholder='Tamaño'></div><div class='col-4'><input name='color' class='form-control' placeholder='Color'></div></div><div class='row g-2 mb-3'><div class='col-6'><input name='proveedor' class='form-control' placeholder='Proveedor'></div><div class='col-6'><input name='observaciones' class='form-control' placeholder='Observaciones'></div></div><button class='btn w-100 btn-lg' style='background:#E70A29;color:#fff;font-weight:800'>Guardar Movimiento</button></form></div><script>const tiendas={json.dumps(tiendas_json)};document.getElementById('regionSelect').addEventListener('change',function(){{let r=this.value;let sel=document.getElementById('tiendaSelect');sel.innerHTML='<option value=>Seleccione Tienda</option>';tiendas.filter(t=>t.region===r).forEach(t=>{{let o=document.createElement('option');o.value=t.tienda;o.textContent=t.tienda;sel.appendChild(o);}})}});</script>"
...
def configuracion():
    ...
    html=f"<div class='container' style='max-width:750px'><div class='card p-3 mb-3 shadow-sm'><h6 class='fw-bold' style='color:#E70A29'>CREAR USUARIOS CON ROL</h6><form method='post' class='row g-2'><input type='hidden' name='action' value='crear_usuario'><div class='col-4'><input name='new_username' class='form-control' placeholder='Usuario' required></div><div class='col-3'><input name='new_password' class='form-control' value='1234' required></div><div class='col-3'><select name='new_role' class='form-select'><option value='operador'>Operador</option><option value='admin'>Admin</option></select></div><div class='col-2'><button class='btn w-100' style='background:#E70A29;color:#fff'>Crear</button></div></form><div class='mt-3'>{lista_u}</div></div><div class='card p-3 mb-3 shadow-sm'><h6 class='fw-bold' style='color:#E70A29'>REGIÓN</h6><form method='post' class='row g-2'><input type='hidden' name='action' value='crear_region'><div class='col-8'><input name='nueva_region' class='form-control' placeholder='Ej: Valle, Costa, etc'></div><div class='col-4'><button class='btn btn-success w-100'>Crear Región</button></div></form></div><div class='card p-3 shadow-sm'><h6 class='fw-bold' style='color:#E70A29'>TIENDA EN REGIÓN</h6><form method='post' class='row g-2'><input type='hidden' name='action' value='crear_tienda'><div class='col-4'><select name='region_id' class='form-select' required><option value=''>Región</option>{opciones}</select></div><div class='col-5'><input name='nueva_tienda' class='form-control' placeholder='Nombre Tienda'></div><div class='col-3'><button class='btn btn-primary w-100'>Crear Tienda</button></div></form></div><a href='/' class='btn btn-secondary w-100 mt-4'>Volver al Inicio</a></div>"
    return render_template_string(BASE, content=html)
...
def borrar_usuario(id):
    if session.get('role')!='admin': return "SOLO ADMIN",403
    execute("DELETE FROM users WHERE id=%s",(id,)); return redirect('/configuracion')
@app.route('/exportar')
@login_required
def exportar():
    movs=fetchall("SELECT * FROM movimientos ORDER BY fecha DESC"); wb=openpyxl.Workbook(); ws=wb.active; ws.append(["Fecha","Region","Tienda","Cantidad","Corregida","Placa","Tipo UCR","Tam","Color","Proveedor","Obs","Usuario"]);
